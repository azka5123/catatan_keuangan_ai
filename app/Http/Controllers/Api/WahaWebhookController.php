...
class WahaWebhookController extends Controller
{
    ...
    {
        $prompt = <<<PROMPT
        Kamu adalah asisten AI keuangan pribadi yang cerdas dan akurat. Tugasmu adalah membantu mencatat transaksi keuangan ke database dan memberikan informasi keuangan saat diminta.
    ...
    private function processAIResponse($result, string $nomorUser, string $today)
    {
        if (preg_match('/```json(.*?)```/s', $result->text(), $matches)) {
            ...
            if (isset($dataArray['action']) && $dataArray['action'] === 'get_data') {
                return $this->handleDataQuery($dataArray, $nomorUser);
